		...
		<div className="flex flex-col gap-6 p-4 md:p-6 overflow-y-auto animate-toastIn">
			...
			<div className="flex flex-col bg-zinc-900 border border-zinc-800 rounded-lg p-5 gap-4 w-full h-80 md:h-100 overflow-y-auto">
				{spreadsheets?.map((spreadsheet, i) => {
					return (
						<button className={`flex justify-between items-center w-full px-3 py-1 cursor-pointer rounded ${(spreadsheet.active) ? "bg-orange-500 text-black" : null} transition duration-1000`} key={i} onClick={() => selectSpreadsheet(socket, spreadsheets, setSpreadsheets, i)}>
