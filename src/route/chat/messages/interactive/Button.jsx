...
function Header({ message }) {
	switch (message.data.interactive.header?.type) {
		case "text":
			return (<p>{message.data.interactive.header.text}</p>);
		case "image":
...
function Footer({ message }) {
	return (
		<span>{message.data.interactive.footer?.text}</span>
	);
}
	...
	return (
		<div className="flex gap-2 text-white">
			{message.data.interactive.action.buttons.map((button, i) => (
				<button className="flex-1 bg-orange-500 rounded hover:bg-orange-400 transition cursor-not-allowed" key={i}>{button.reply.title}</button>
			))}
		</div>
	);
}
